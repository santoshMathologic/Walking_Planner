		...
		return {
			...
			link: function (scope, elem, attrs, controller) {
				...
				var itemTemplate = elem.html().trim() || '{{' + (match[2] || match[1]) + '}}',
					...
					dropdownTemplate =
					'<a id="selectDropDown" class="form-control input-md" data-toggle="dropdown" href ng-class="{ disabled: disabled }">' +
						'<span ng-hide="customSelectDropDown">{{displayText}}</span>' +
						'<span ng-show="customSelectDropDown">{{customSelectDropDown}}</span>' +
						'<b></b>' +
					'</a>' +
