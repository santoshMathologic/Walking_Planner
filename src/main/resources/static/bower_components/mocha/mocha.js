...
function List(runner) {
  ...
  runner.on('fail', function(test, err){
    (JSON.stringify(['fail', clean(test)]));
  });
...
function TAP(runner) {
  ...
  runner.on('fail', function(test, err){
    ...
    if (err.stack) (err.stack.replace(/^/gm, '  '));
  });
...
function test(test) {
  ...
  if ('failed' == test.state) {
    ...
    (tag('testcase', attrs, false, tag('failure', attrs, false, cdata(err.stack))));
  } else if (test.pending) {
    (tag('testcase', attrs, false, tag('skipped', {}, true)));
  } else {
    (tag('testcase', attrs, true) );
  }
}
